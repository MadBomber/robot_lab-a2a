...
module AdapterTestHelpers
  ...
  class MockTask
    attr_reader :state, :artifacts, :last_message
    ...
    def complete!(**o)  = (@state = :completed;     @artifacts    = o[:artifacts] || [])
    def fail!(**o)      = (@state = :failed;         @last_message = o[:message])
    def cancel!         = (@state = :cancelled)
    def require_input!(**o) = (@state = :input_required; @last_message = o[:message])
  end
  ...
  class MockNetwork
    def run(message:) = MockNetworkResult.new
  end
end

# ── RobotAdapter — :none mode ─────────────────────────────────────────────────

class RobotLab::A2A::RobotAdapterNoneTest < Minitest::Test
...
class RobotLab::A2A::RobotAdapterA2AToolTest < Minitest::Test
  include AdapterTestHelpers
  ...
  def test_done_path_restores_tools
    ...
    assert_empty robot.local_tools.select { |t| t.is_a?(RobotLab::A2A::AskUserTool) }
  end
...
class RobotLab::A2A::RobotAdapterIoBridgeTest < Minitest::Test
  include AdapterTestHelpers
...
class RobotLab::A2A::NetworkAdapterTest < Minitest::Test
  include AdapterTestHelpers
